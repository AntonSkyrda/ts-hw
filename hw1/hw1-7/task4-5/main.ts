...
class Client {
    ...
    constructor(id: number, name: string, surname: string, email: string, phone: number, order: OrderType[]) {
    this.id = id;
    this.name = name;
...
let sortedClientsByOrderList: Client[] = clients.sort((a:Client, b:Client) => a.order.length - b.order.length);
